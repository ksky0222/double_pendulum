...
if mode==0:
    ...
    # 저장
    plt.savefig('plot_img/Evaluate_Mean_Reward.png')
    plt.show()
elif mode==1:
        # CSV 파일 경로
    ...
    # 저장
    plt.savefig('plot_img/Actor Loss.png')
    plt.show()
elif mode==3:
    # CSV 파일 경로
    ...
    # 저장
    plt.savefig('plot_img/Critic Loss.png')
    plt.show()
    ...
    # LIPS CSV
    df_lips = pd.read_csv('plot_data/lips_req_mean2.csv')
    # df_lips = df_lips[df_lips['Step'] <= 30_000_000]
    df_lips = df_lips[df_lips['Step'] <= 75_000_000]
    x_lips = df_lips['Step']
    ...
    df_sac = df_sac[df_sac['Step'] <= 75_000_000]
    x_sac = df_sac['Step']
    y_sac = df_sac['Value']
    ...
    # 저장
    plt.savefig('plot_img/Compare_Episode_Mean_Reward.png')
    plt.show()

elif mode==7:
    # LIPS CSV
    df_lips = pd.read_csv('plot_data/lips_eval_mean2.csv')
    # df_lips = df_lips[df_lips['Step'] <= 30_000_000]
    df_lips = df_lips[df_lips['Step'] <= 75_000_000]
    x_lips = df_lips['Step']
    y_lips = df_lips['Value']
    ...
    # 저장
    plt.savefig('plot_img/Compare_Evaluation_Mean_Reward.png')
    plt.show()